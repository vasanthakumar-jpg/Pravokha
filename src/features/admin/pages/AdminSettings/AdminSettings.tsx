...
import { AdminFormSkeleton } from "@/features/admin/components/AdminSkeleton";
  ...
  return (
    <div className="w-full mx-auto py-8 px-4 sm:px-6 lg:px-8 flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
            <Button
              variant="outline"
              size="sm"
              className="h-9 rounded-xl border-border/40 bg-card gap-2 font-bold text-xs w-fit justify-start"
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col xl:flex-row gap-8">
        {/* Sidebar Navigation (Hidden < XL) */}
        <aside className="hidden xl:block xl:w-64 shrink-0">
          <TabsList className={cn(
            "flex xl:flex-col h-auto w-full bg-card p-3 border border-border/60 rounded-2xl gap-2 !items-stretch",
            "overflow-x-auto xl:overflow-x-visible no-scrollbar pb-3 xl:pb-3"
          )}>
            {[
              ...
              <TabsTrigger
                ...
                value={tab.id}
                className={cn(
                  "flex-shrink-0 xl:w-full !justify-start px-4 py-3 h-auto rounded-xl transition-all duration-500 relative group overflow-hidden",
                  "data-[state=active]:bg-primary data-[state=active]:text-white",
                  "hover:bg-primary/5 text-muted-foreground font-bold whitespace-nowrap text-left border border-transparent data-[state=active]:border-white/10"
                )}
              >
                <div className="flex items-center gap-4 py-1">
                  <div className={cn(
                    "h-10 w-10 rounded-xl flex items-center justify-center transition-all duration-300",
                    "bg-muted/40 text-muted-foreground group-data-[state=active]:bg-white/20 group-data-[state=active]:text-white shadow-inner"
                  )}>
                    <tab.icon className="h-5 w-5" />
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[11px] font-bold tracking-tight uppercase">{tab.label}</span>
                    <span className="text-[10px] font-medium opacity-50 group-data-[state=active]:opacity-80 tracking-tighter">{tab.description}</span>
                  </div>
                </div>
                {/* Active Indicator Pulse */}
                <TabsContent value={tab.id}>
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 h-1.5 w-1.5 rounded-full bg-white shadow-glow animate-pulse" />
                </TabsContent>
              </TabsTrigger>
            ))}
          </TabsList>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              className="rounded-xl border-border/60 h-11 text-sm font-bold bg-muted/20 focus:ring-primary/20 transition-all placeholder:text-muted-foreground/30 px-4"
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              className="rounded-xl border-border/60 h-11 text-sm font-bold bg-muted/20 focus:ring-primary/20 transition-all px-4"
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              className="rounded-xl border-border/60 h-11 text-sm font-bold bg-muted/20 focus:ring-primary/20 transition-all px-4"
                              placeholder="Regional jurisdiction"
                            />
                          </div>
                        </div>
                      </CardContent>
                  ...
                  <Card className="border-rose-500/20 bg-rose-500/5 rounded-2xl overflow-hidden mt-6">
                    <CardContent className="p-6 flex flex-col md:flex-row items-center justify-between gap-6">
                      ...
                      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
                        ...
                        <AlertDialogContent className="rounded-2xl border-rose-500/20 bg-background/95 backdrop-blur-xl max-w-lg">
                          <AlertDialogHeader>
