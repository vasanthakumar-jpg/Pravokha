...
export const vendorSettingsSchema = z.object({
    ...
    // Bank Details
    bankAccountNumber: z.preprocess((val) => (val === '' ? null : val), z.string().min(5, 'Account number is too short').optional().nullable()),
    bankIfscCode: z.preprocess((val) => (val === '' ? null : val), z.string().min(4, 'IFSC code is too short').optional().nullable()),
    beneficiaryName: z.preprocess((val) => (val === '' ? null : val), z.string().min(2, 'Beneficiary name is too short').optional().nullable()),
    bankName: z.preprocess((val) => (val === '' ? null : val), z.string().min(2, 'Bank name is too short').optional().nullable()),
    bankAccountName: z.preprocess((val) => (val === '' ? null : val), z.string().min(2, 'Account holder name is too short').optional().nullable()),
